...
function collectSourceHeadlines(name, rawTweets, rules, config) {
  ...
  Object.keys(rawTweets).forEach((tweet) => {
    ...
    const this_source_screen_name = ('screen_name' in rawTweets[tweet].user) ? rawTweets[tweet].user.screen_name : '';
    headline.source_url = ('url' in rawTweets[tweet].user) ? rawTweets[tweet].user.url : null;
    if (headline.source_url == null) {
      headline.source_url = "https://twitter.com/" + this_source_screen_name;
    }
    if (headline.source_name == "All Apple, Always | News, Reviews, Guides") {
      jsonfile.writeFile('rawTweet.json', rawTweets[tweet]);
    };
