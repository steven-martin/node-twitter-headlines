...
function collectSourceHeadlines(name, rawTweets, rules, config) {
  ...
  Object.keys(rawTweets).forEach((tweet) => {
    ...
    headline.source_name =
      'name' in rawTweets[tweet].user ? rawTweets[tweet].user.name : '';
    headline.source_photo =
      'profile_image_url' in rawTweets[tweet].user
        ? rawTweets[tweet].user.profile_image_url
        : '';
    const this_source_screen_name =
      'screen_name' in rawTweets[tweet].user
        ? rawTweets[tweet].user.screen_name
        : '';
    headline.source_url =
      'url' in rawTweets[tweet].user ? rawTweets[tweet].user.url : null;
    if (headline.source_url == null) {
      headline.source_url = 'https://twitter.com/' + this_source_screen_name;
    }

    // collect the article fields
    headline.article_link =
      0 in rawTweets[tweet].entities.urls
        ? rawTweets[tweet].entities.urls[0].expanded_url
        : '';
    headline.article_photo =
      'extended_entities' in rawTweets[tweet]
        ? rawTweets[tweet].extended_entities.media[0].media_url
        : '';

    // collect and tidy the article description
    headline.article_description =
      'text' in rawTweets[tweet] ? rawTweets[tweet].text : '';
    headline.article_description = headline.article_description.replace(
      /(?:https?|ftp):\/\/[\n\S]+/g,
      ''
    );
    headline.article_description = headline.article_description.replace(
    ...
    headline.date =
      'created_at' in rawTweets[tweet] ? rawTweets[tweet].created_at : '';
    const timestamp = Date.parse(headline.date);
    headline.timestamp = timestamp;

    // collect and calculate the headline score
    const retweetCount =
      'retweet_count' in rawTweets[tweet] ? rawTweets[tweet].retweet_count : 0;
    const favoriteCount =
      'favorite_count' in rawTweets[tweet] === true
        ? rawTweets[tweet].favorite_count
        : 0;
    ...
    headline.score = parseInt(
      reverseAgeScore + retweetCount / 10 + favoriteCount / 5,
      10
    );
    ...
    Object.keys(categoryList).forEach((category) => {
      const searchPattern = new RegExp(
        categoryList[category].search_pattern,
        'i'
      );
    ...
    if (headline.article_description === '') {
      include = false;
    }
    if (headline.article_link === '') {
      include = false;
    }
    if (headline.article_photo === '') {
      include = false;
    }
...
function getHeadlines(thisInstance) {
  return new Promise(() => {
    ...
    const process = async () => {
      await asyncLoop(sources, async (source) => {
        ...
        await twitter
          ...
          .then((res) => {
            ...
            const thisSourceHeadlines = collectSourceHeadlines(
              res.slug,
              res.data,
              res.rules,
              thisInstance.config
            ...
            Object.keys(thisSourceHeadlines).forEach((thisSourceHeadlineID) => {
              ...
              thisInstance.headlines.push(
                thisSourceHeadlines[thisSourceHeadlineID]
              );
              // adds the current headline to thisInstance.category, if description doesn't exist in any of the existing articles
              let alreadyexists = false;
              const compareDescription1 =
                thisSourceHeadlines[thisSourceHeadlineID].article_description;
              Object.keys(
                thisInstance.categories[
                  thisSourceHeadlines[thisSourceHeadlineID].category
                ]
              ).forEach((thisCategory) => {
                const compareDescription2 =
                  thisInstance.categories[
                    thisSourceHeadlines[thisSourceHeadlineID].category
                  ][thisCategory].article_description;
              ...
              if (alreadyexists === false) {
                thisInstance.categories[
                  thisSourceHeadlines[thisSourceHeadlineID].category
                ].push(thisSourceHeadlines[thisSourceHeadlineID]);
              }
            });
      ...
      Object.keys(thisInstance.config.engine.categories).forEach(
        (categoryID) => {
          // sort the category - only by timestamp
          thisInstance.categories[
            thisInstance.config.engine.categories[categoryID].category
      ...
      thisInstance.categories.News.sort(
        (a, b) => parseFloat(b.score) - parseFloat(a.score)
      );
      // trim category to a max of 20
